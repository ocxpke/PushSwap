...
/*   Updated: 2024/12/29 22:02:44 by jose-ara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*final_ret(char **ptr, char *str_sta, int pos_nl, ssize_t cont)
{
	...
	concat_str(aux, *ptr, ft_strlen(*ptr));
...
static void	ft_realloc(char **ptr, char *str_sta, int pos_nl)
{
	...
	if (pos_nl == -1)
		len = ft_strlen_line(str_sta);
	else
		len = pos_nl;
	if (*ptr)
		len += ft_strlen_line(*ptr);
	aux = (char *)malloc(len + 1);
	if (!aux)
		return ;
	set_zero(aux, len + 1);
	concat_str(aux, *ptr, ft_strlen_line(*ptr));
	concat_str(aux, str_sta, len - ft_strlen_line(*ptr));
	free(*ptr);
	reset_str_sta(str_sta, pos_nl);
	*ptr = aux;
}
...
char	*get_next_line(int fd)
{
	...
	else if (ft_strlen_line(str_read) > 0)
