...
/*   Updated: 2024/12/29 22:02:29 by jose-ara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	reset_str_sta(char *str_sta, int pos_nl)
{
	...
	aux = (char *)malloc(ft_strlen_line(str_sta) + 1);
	if (!aux)
		return (set_zero(str_sta, BUFFER_SIZE));
	set_zero(aux, ft_strlen_line(str_sta) + 1);
	iter = -1;
	while ((iter++) < (ft_strlen_line(str_sta) - pos_nl))
		aux[iter] = str_sta[iter + pos_nl];
	set_zero(str_sta, BUFFER_SIZE);
	iter = -1;
	while ((iter++) < ft_strlen_line(aux))
